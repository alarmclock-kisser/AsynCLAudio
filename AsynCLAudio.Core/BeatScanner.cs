...
namespace AsynCLAudio.Core
{
	...
	{
		private static ConcurrentDictionary<AudioObj, List<float>> scannedTimings = [];
		private static ConcurrentDictionary<AudioObj, List<double>> scannedBpms = [];
		...
		public static async Task<double> ScanBpmAsync(AudioObj obj, int windowSize = 65536, int lookingRange = 2, int minBpm = 60, int maxBpm = 200, bool autoGetTiming = false)
		{
			...
			var monoData = await obj.GetCurrentWindow(windowSize, lookingRange, true, true);
			...
			{
				List<float> timings = [];
				if (scannedTimings.TryGetValue(obj, out var t))
				{
					timings = t;
					timing = t.Last();
				}
			...
			// Check if BPM already scanned
			if (!scannedBpms.TryGetValue(obj, out var bpms))
			{
				scannedBpms.AddOrUpdate(obj, [bpm], (key, existing) =>
				{
					existing.Add(bpm);
					return existing;
				});
		...
		public static async Task<float> ScanTimingAsync(AudioObj obj, int windowSize = 65536, int lookingRange = 2)
		{
			...
			var monoData = await obj.GetCurrentWindow(windowSize, lookingRange, true, false);

			float timing = await EstimateTimingAsync(monoData, obj.SampleRate);

			// Check if timing already scanned
			if (!scannedTimings.TryGetValue(obj, out var timings))
			{
				scannedTimings.AddOrUpdate(obj, [timing], (key, existing) =>
				{
					existing.Add(timing);
					return existing;
				});
		...
		public static async Task<float> EstimateTimingAsync(float[] samples, int sampleRate)
		{
			...
			return await Task.Run(() =>
			{
				...
				for (int idx = 0; idx < candidates.Length; idx++)
				{
					...
					if (K == 2) score *= 0.92;      // 2/4 leicht benachteiligen
					if (K == 4) score *= 1.04;      // 4/4 leicht bevorzugen
					if (K % 3 == 0) score *= 1.06;  // 3er-Gruppen (3/4, 6/8, 12/8) fördern
				...
				{
					normalized = bestK / 8.0;
				}
				else
				{
					normalized = bestK / 12.0;
				}
		...
		private static float GetMedianTiming(List<float> timings)
		{
			...
			timings.Sort();
			int mid = Math.Clamp(timings.Count / 2, 0, timings.Count - 1);

			return timings[mid];
		}

		public static void RemoveScanned(AudioObj audioObj)
		{
			// Try to remove scanned timings and BPMs for the given audio object
			if (audioObj == null)
			{
				return;
			}

			if (scannedTimings.ContainsKey(audioObj))
			{
				scannedTimings.TryRemove(audioObj, out _);
			}

			if (scannedBpms.ContainsKey(audioObj))
			{
				scannedBpms.TryRemove(audioObj, out _);
			}
		}

		public static float GetLastTiming(AudioObj audioObj)
		{
			if (audioObj == null || !scannedTimings.TryGetValue(audioObj, out var timings) || timings.Count == 0)
			{
				return -1.0f;
			}
		...
		public static double GetLastBpm(AudioObj audioObj)
		{
			if (audioObj == null || !scannedBpms.TryGetValue(audioObj, out var bpms) || bpms.Count == 0)
			{
				return -1.0f;
			}
